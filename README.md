...
To run algorithm for allocation racks and devices after creating them inside database (using CRUD operations for example) you can go to http://0.0.0.0:8000/devicerackallocation and put device and racks serial numbers together with comma (do not separate them with space, comma is the only separator) which are primary keys inside database.
